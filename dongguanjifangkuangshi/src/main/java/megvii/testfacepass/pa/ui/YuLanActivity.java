...
public class YuLanActivity extends Activity implements CameraManager.CameraListener, View.OnClickListener ,CameraManager2.CameraListener2{
    ...
    private Box<Subject> faceIDBeanBox  = MyApplication.myApplication.getSubjectBox();
    ...
    /* 相机是否使用前置摄像头 */
    private String tupianFaceToken2 = null;
    ...
    private FacePassHandler paAccessControl=MyApplication.myApplication.getFacePassHandler();
...
//            if (rgb == null || !configBean.isHuoTi())
//                return;
...
//        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_group_name:
                ...
                if (paAccessControl == null) {
                    Toast tastyToast = Toast.makeText(YuLanActivity.this, "算法未初始化", Toast.LENGTH_LONG);
                    tastyToast.setGravity(Gravity.CENTER, 0, 0);
                    tastyToast.show();
                    return;
                }

                if (tupianFaceToken2 != null) {
                    try {
                        paAccessControl.deleteFace(tupianFaceToken2.getBytes());
                        tupianFaceToken2 = null;
                ...
                try {
                    ...
                    if (detectionResult == null || detectionResult.faceList.length <= 0) {
                        ...
                        if (isOPEN){
                            ...
                            Log.d("FeedFrameThread", "detectionResult.images:" + detectionResult.images.length);
                            final FacePassFace[] bufferFaceList = detectionResult.faceList;
                            if (bufferFaceList.length>0){

                                bitmap=nv21ToBitmap.nv21ToBitmap(image.image,image.width,image.height);
                                bitmap = BitmapUtil.rotateBitmap(bitmap, SettingVar.msrBitmapRotation);

                                Log.d("FeedFrameThread", "bitmap:" + bitmap);
                                runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        try {
                                            FacePassAddFaceResult result = paAccessControl.addFace(bitmap);
                                            if (result != null) {
                                                if (result.result == 0) {
                                                    tupianFaceToken2 = new String(result.faceToken);
                                                    //5rMQGDrjyQdgNZFmgxtFJA==
                                                    Log.d("tupianFaceToken2", "" + tupianFaceToken2);
                                                    toast("图片质量通过");
    ...
    private AlertDialog faceAddDialog;

    private void showFaceAddDialog(Bitmap fileBitmap) {


        if (faceAddDialog != null && !faceAddDialog.isShowing()) {
            faceAddDialog.show();
            return;
        }
        if (faceAddDialog != null) {
            ImageView ii = faceAddDialog.findViewById(R.id.touxiang);
            ii.setImageBitmap(fileBitmap);
            return;
        }

        AlertDialog.Builder builder = new AlertDialog.Builder(this, R.style.TransparentDialog);
        View view = LayoutInflater.from(this).inflate(R.layout.layout_dialog_face_operation22, null);
        ...
        queding.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (name.getText().toString().trim().equals("")) {
                    ...
                    if (tupianFaceToken2 == null) {
                        toast("人脸方向不对或质量不符合要求");
                    } else {
                        //开始添加
                        try {
                            boolean b = paAccessControl.bindGroup(group_name, tupianFaceToken2.getBytes());
                            if (b) {
                                long  personId =System.currentTimeMillis();
                                Subject faceIDBean=subjectBox.query().equal(Subject_.teZhengMa,tupianFaceToken2).build().findUnique();
                                if (faceIDBean==null){
                                    Subject fff = new Subject();
                                    fff.setTeZhengMa(tupianFaceToken2);
                                    fff.setId(System.currentTimeMillis());
                                    fff.setSid(personId+"");
                                    fff.setIsOpen(0);
                                    fff.setWorkNumber("12345678");
                                    fff.setBirthday("2029-12-12 12:12:12");
                                    fff.setName(name.getText().toString().trim() + "");
                                    subjectBox.put(fff);
                                }else {
                                    paAccessControl.deleteFace(faceIDBean.getTeZhengMa().getBytes());
                                    faceIDBean.setTeZhengMa(tupianFaceToken2);
                                    faceIDBean.setName(name.getText().toString().trim() + "");
                                    subjectBox.put(faceIDBean);
                                }
                                tupianFaceToken2 = null;
                                toast("入库成功");
                                faceAddDialog.dismiss();
                                faceAddDialog.cancel();
                                faceAddDialog = null;
                            } else {
                                Log.d("", "入库失败");
                                toast("入库失败,请重试");
        ...
        faceAddDialog = builder.create();
        faceAddDialog.setCanceledOnTouchOutside(false);
        faceAddDialog.show();

        WindowManager.LayoutParams attributes = faceAddDialog.getWindow().getAttributes();
        attributes.gravity = Gravity.CENTER;
        attributes.height = (int) (height * 0.68f);
        attributes.width = (int) (width * 0.6f);
        faceAddDialog.getWindow().setAttributes(attributes);
...
//    }
    ...
    private void toast(String msg) {
        Toast tastyToast = Toast.makeText(YuLanActivity.this, msg, Toast.LENGTH_LONG);
        tastyToast.setGravity(Gravity.CENTER, 0, 0);
        tastyToast.show();
